...
function Enable-GbExecuter {
    <#
    .SYNOPSIS
        Asegura que la variable $script:Executer esté cargada.
    #>
    if (-not $script:Executer) {
        try {
            Write-Verbose "Cargando variable Executer desde GitHub..."
            $url = "https://raw.githubusercontent.com/gbelarbide/SC-online/refs/heads/main/Deploy/Executer.ps1"
            $content = (New-Object Net.WebClient).DownloadString($url)
            # El script Executer.ps1 define la variable $Executer
            $null = Invoke-Expression $content
            if ($null -ne $Executer) { 
                $script:Executer = $Executer 
                Write-Verbose "Executer cargado correctamente ($($script:Executer.Length) bytes)"
            }
        }
        catch {
            Write-GbLog -Message "No se pudo cargar Executer de GitHub: $_" -Level "ERROR"
        }
    }
}
...
function Show-InstallationProgress {
    ...
    try {
        ...
        # Asegurar Executer cargado
        Enable-GbExecuter
        if (-not $script:Executer) {
            throw "La variable Executer no está definida y no pudo cargarse."
        }
        ...
        $bytes = [Convert]::FromBase64String($script:Executer)
        ...
        if (-not $isSystem) {
            ...
            foreach ($session in $sessions) {
                ...
                if ($sessionId) {
                    ...
                    for ($i = 0; $i -lt 20; $i++) {
                        $processInfo = Get-CimInstance Win32_Process -Filter "Name = 'Executer.exe'" | 
                    ...
                    else { Write-Warning "No se pudo detectar el proceso Executer.exe iniciado" }
                    break
                }
            }
        }
...
function Close-InstallationProgress {
    ...
    try {
        ...
        # 1. Intentar por ID de proceso si lo tenemos
        ...
        # 2. Búsqueda de seguridad por Command Line (si el PID falló o es nulo)
        if ($ProgressInfo.ScriptPath) {
            Write-Verbose "Buscando procesos por script para cierre: $($ProgressInfo.ScriptPath)"
            $procs = Get-CimInstance Win32_Process -Filter "Name = 'Executer.exe'" | 
            Where-Object { $_.CommandLine -like "*$($ProgressInfo.ScriptPath)*" }
            
            foreach ($p in $procs) {
                Write-Verbose "Cerrando proceso de seguridad PID: $($p.ProcessId)"
    ...
    try {
        ...
        if ($isLastAttempt) {
            ...
            if ($deployResult.Success) {
                ...
                Show-UserPrompt -Message "La instalacion de $Name Finalizo Correctamente" -Title "Instalacion de $Name" -Buttons "OK" -Icon "Information" | Out-Null
            }
            else {
            ...
            if ($isFirstRun) {
                ...
                # No incrementar el contador en la programacion inicial para que el primer dialogo sea el Intento 1
                $nextAttempt = 1
...
"@)
                ...
                if (-not $hasActiveSessions) {
                    ...
                    if ($deployResult.Success) {
                        Write-GbLog -Message "Despliegue de $Name completado exitosamente." -Level "SUCCESS"
                        Show-UserPrompt -Message "La instalacion de $Name Finalizo Correctamente" -Title "Instalacion de $Name" -Buttons "OK" -Icon "Information" -TimeoutSeconds 30 | Out-Null
